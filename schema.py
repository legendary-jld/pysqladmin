...
class schema:
    ...
    def load(self):
        ...
        for db in databases:
            ...
            table_dict = {}
            for table in tables:
                # columns = self.localdb.query("SELECT id,column_name FROM column_store WHERE session_uid=:uid;", {"uid": self.session_uid})
    ...
    def mysql_refresh_dbs(self, recursive=False):
        clean_table = self.localdb.execute("DELETE FROM db_store WHERE session_uid=:uid;", {"uid": self.session_uid})
        ...
        if recursive:
            dbs = self.localdb.query("SELECT id,db_name FROM db_store WHERE session_uid=:uid;", {"uid":self.session_uid})
            if dbs:
                for db in dbs:
    ...
    def mysql_refresh_tables(self, db, recursive=True):
        clean_table = self.localdb.execute(
        ...
        if recursive:
            tables = self.localdb.query("SELECT id,table_name FROM table_store WHERE session_uid=:uid", {"uid":self.session_uid})
            for table in tables:
                self.mysql_refresh_columns(table, recursive=True)

    # column_store (id integer, session_uid text, created text, table_id, integer, field_name text)
    def mysql_refresh_columns(self, table, recursive=True):
        # clean_table = self.localdb.execute(
